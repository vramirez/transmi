...
sqlContext.sql("select created_at,cast ( from_unixtime( unix_timestamp(concat( substring(created_at,27,4),' ', substring(created_at,5,15)), 'yyyy MMM dd hh:mm:ss')) as timestamp) ts from predicts where created_at like '% 12:%'").show()
